...
export function BettingSiteCard({ site, rank }: BettingSiteCardProps) {
  const getPrimaryBadge = () => {
    if (site.isTopRating)
      return {
        text: "RECOMMANDÉ",
        color: "bg-gradient-to-r from-purple-600 to-indigo-600 text-white border-2 border-white shadow-xl",
      }
    if (site.isTopLine)
      return {
        text: "MEILLEUR CHOIX",
    ...
    if (site.isPopular)
      return {
        text: "POPULAIRE",
        color: "bg-gradient-to-r from-emerald-500 to-green-600 text-white border-2 border-white shadow-xl",
      }
    if (site.isTrending)
      return {
        text: "TENDANCE",
        color: "bg-gradient-to-r from-rose-500 to-pink-600 text-white border-2 border-white shadow-xl",
      }
    return null
  }
  ...
  return (
    ...
    >
      <div
        className={`relative rounded-2xl p-6 transition-all duration-300 group-hover:scale-[1.02] shadow-xl border-2 ${
          isFirstItem
            ? "bg-gradient-to-br from-purple-50 via-white to-indigo-50 border-purple-400 shadow-2xl shadow-purple-400/20"
            : "bg-gradient-to-br from-slate-50 via-white to-gray-50 border-slate-300 hover:border-slate-400"
        }`}
      >
        {primaryBadge && (
        ...
        <div className="absolute inset-0 rounded-2xl overflow-hidden">
          {isFirstItem ? (
            <>
              ...
              <div className="absolute -bottom-8 -left-8 w-24 h-24 rounded-full bg-gradient-to-br from-purple-200/20 to-indigo-300/15"></div>
              <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-gradient-to-br from-indigo-200/20 to-purple-300/15"></div>
            </>
          ) : (
            <>
              <div className="absolute top-6 left-6 w-5 h-5 opacity-8">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-teal-600">
              ...
              <div className="absolute -bottom-6 -left-6 w-20 h-20 rounded-full bg-gradient-to-br from-teal-200/15 to-cyan-300/10"></div>
              <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full bg-gradient-to-br from-slate-200/15 to-gray-300/10"></div>
            </>
          )}
        </div>
          ...
          <div className="hidden md:flex md:items-center md:gap-4 lg:gap-6 pt-2">
            <div
              className={`w-14 h-14 font-bold rounded-full flex items-center justify-center text-2xl flex-shrink-0 border-4 shadow-lg ${
                isFirstItem
                  ? "bg-gradient-to-r from-purple-600 to-indigo-600 text-white border-white"
                  : "bg-gradient-to-r from-teal-700 to-cyan-800 text-white border-white"
              }`}
            >
              {rank}
            </div>
            ...
            <div className="flex-1 text-center">
              ...
              <p
                className={`font-semibold leading-tight text-2xl ${isFirstItem ? "text-indigo-800" : "text-teal-600"}`}
              >
                {site.bonus}
              </p>
            </div>
                ...
                {[...Array(5)].map((_, i) => (
                  <Star
                    ...
                    className={`w-4 h-4 stroke-[1.5px] ${
                      i < site.stars ? "text-amber-500 fill-amber-500" : "text-transparent stroke-slate-400"
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="flex-shrink-0">
              <Button
                className={`font-bold px-6 py-3 text-base border-2 flex items-center gap-2 shadow-lg transition-all hover:scale-105 ${
                  isFirstItem
                    ? "bg-purple-600 hover:bg-purple-700 text-white font-bold border-indigo-500"
                    : "bg-teal-600 hover:bg-teal-700 text-white font-bold border-cyan-500"
                }`}
              >
                VISITER SITE
          ...
          <div className="md:hidden pt-2">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex-1 text-center pl-4">
                ...
                <p
                  className={`font-semibold leading-tight text-lg ${isFirstItem ? "text-indigo-800" : "text-teal-600"}`}
                >
                  {site.bonus}
                </p>
              </div>
