...
export function BettingSiteCard({ site, index, isTopChoice = false }: BettingSiteCardProps) {
  const rank = index + 1

  const safeRating = typeof site.rating === "number" && !isNaN(site.rating) ? site.rating : 0
  const safeStars = typeof site.stars === "number" && !isNaN(site.stars) ? site.stars : 0
  const safeReviews = typeof site.reviews === "number" && !isNaN(site.reviews) ? site.reviews : 0
  ...
  const getPrimaryBadge = () => {
    ...
    if (site.isTopLine)
      return {
        text: "COTES PREMIUM",
        color: "bg-gradient-to-r from-teal-600 to-cyan-600 text-white border-2 border-white shadow-xl",
      }
    if (site.isPopular)
      return {
        text: "FAVORI",
        color: "bg-gradient-to-r from-amber-500 to-yellow-500 text-white border-2 border-white shadow-xl",
      }
    if (site.isTrending)
      return {
        text: "EN VOGUE",
        color: "bg-gradient-to-r from-indigo-600 to-blue-600 text-white border-2 border-white shadow-xl",
      }
    return null
  }
    ...
    >
      <div
        className={`relative rounded-xl p-6 transition-all duration-300 group-hover:scale-[1.02] shadow-xl border-2 ${
          isFirstItem
            ? "bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 border-amber-400 shadow-2xl shadow-amber-400/40 ring-2 ring-amber-300/50"
            : "bg-gradient-to-br from-purple-50 via-white to-indigo-50 border-slate-300 hover:border-slate-400"
        }`}
      >
        {/* Badge - repositionné en haut à gauche */}
        {primaryBadge && (
        ...
        <div className="absolute inset-0 rounded-xl overflow-hidden">
          {isFirstItem ? (
            <>
              ...
              {/* Enhanced golden circles */}
              <div className="absolute -bottom-8 -left-8 w-24 h-24 rounded-full bg-gradient-to-br from-amber-300/30 to-orange-300/25"></div>
              <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-gradient-to-br from-yellow-300/30 to-amber-300/25"></div>
              {/* Additional golden glow effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-amber-100/20 via-transparent to-orange-100/20 rounded-xl"></div>
            </>
          ) : (
            <>
          ...
          <div className="hidden md:flex md:items-center md:gap-4 lg:gap-6 pt-2">
            ...
            <div
              className={`w-14 h-14 font-bold rounded-full flex items-center justify-center text-2xl flex-shrink-0 border-4 shadow-lg ${
                isFirstItem
                  ? "bg-gradient-to-r from-amber-500 via-yellow-500 to-orange-500 text-white border-white shadow-amber-400/50"
            ...
            <div className="flex-1 text-center">
              <div className={`font-bold mb-1 text-xl ${isFirstItem ? "text-amber-700" : "text-slate-700"}`}>
                BONUS INFORMATIONS
              </div>
              <p
                className={`font-semibold leading-tight text-2xl ${isFirstItem ? "text-orange-700" : "text-green-600"}`}
              >
                {site.bonus}
              </p>
            </div>
            ...
            <div className="text-center flex-shrink-0">
              <div
                className={`text-xl lg:text-2xl font-bold mb-1 ${isFirstItem ? "text-amber-700" : "text-slate-700"}`}
              >
                ...
                {[...Array(5)].map((_, i) => (
                  <Star
                    ...
                    className={`w-4 h-4 stroke-[1.5px] ${
                      i < safeStars ? "text-yellow-500 fill-yellow-500" : "text-transparent stroke-slate-400"
                    }`}
                  />
                ))}
              </div>
              <div className="text-slate-500 text-xs">
                <span>({safeReviews} avis)</span>
              </div>
            </div>
            ...
            <div className="flex-shrink-0">
              <Button
                className={`font-bold px-6 py-3 text-base border-2 flex items-center gap-2 shadow-lg transition-all hover:scale-105 ${
                  isFirstItem
                    ? "bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-bold border-yellow-400 shadow-amber-400/50"
          ...
          <div className="md:hidden pt-2">
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex-1 text-center pl-4">
                <div className={`font-bold text-sm mb-1 ${isFirstItem ? "text-amber-700" : "text-slate-700"}`}>
                  BONUS INFO
                </div>
                <p
                  className={`font-semibold leading-tight text-lg ${isFirstItem ? "text-orange-700" : "text-green-600"}`}
                >
                  {site.bonus}
                </p>
              </div>
            ...
            <div className="flex items-center justify-center gap-4 mb-4">
              <div className="flex items-center gap-2">
                <span className={`font-bold text-lg ${isFirstItem ? "text-amber-700" : "text-slate-700"}`}>
                  {safeRating.toFixed(1)}
                  <span className="text-sm text-slate-500">/10</span>
                </span>
...
export { BettingSiteCard as BettingCard }
